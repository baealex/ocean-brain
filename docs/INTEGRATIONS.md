...
Migration `0020` creates the platform records under their original names. Migration `0021` renames them to `IntegrationConnection` and `IntegrationCredential`, including the `integrationId` and `connectionId` columns, without replacing IDs, manifests, grants, enabled/pinned states, credentials, or timestamps. Migration `0022` adds nullable private proxy URL and latest status report fields; existing connections and tokens are preserved. These migrations run automatically; installing or upgrading an individual integration app does not add another core migration.
