...
- The source of truth is `oceanBrain.mcpCompatibilityVersion` in `packages/cli/package.json`. Both the server and bundled MCP adapter read this metadata. The package's `version` identifies the app release; the MCP compatibility value identifies its tool/API contract, not a minimum installable package version.
- Check whether the change breaks an existing MCP client contract: removed or renamed tools/fields, incompatible input/output shapes, changed authentication/header requirements, or incompatible read/write behavior. Additive tools or optional fields that preserve existing contracts do not by themselves require a bump.
- If compatibility is preserved, keep the value unchanged, even when the app receives a minor or major release.
- Before the app reaches `1.0.0`, align an incompatible MCP contract with the target app minor release so support and migration guidance use one version, for example app `0.13.3` → `0.14.0` and MCP compatibility `0.11.0` → `0.14.0`. Compatible app patch releases keep the existing MCP compatibility value. Revisit the compatibility numbering policy as part of the `1.0.0` release contract.
- The current guard requires matching compatibility major and minor numbers and ignores the patch number. Increasing only the patch does not block incompatible clients. A numerically newer compatibility version is not automatically compatible either.
- Make the compatibility change in the feature PR that changes the contract. Update metadata assertions, affected fixtures, migration documentation, and tool replacements together. The separate release PR bumps only the app package version and retains the compatibility value already reviewed on `main`.
- Document the old and new compatibility values and the required client/server upgrade in both the feature PR and release PR. If the tool catalog changes, tell users to reconnect their MCP client so it reloads the catalog. Classify release impact using `GIT_CONVENTION.md`.
