...
- For the next incompatible contract, increment the compatibility minor number and reset its patch to zero, for example `0.11.0` → `0.12.0`. Do not derive this number from the app release version. Change the compatibility major number only as part of an explicitly documented compatibility-generation change.
