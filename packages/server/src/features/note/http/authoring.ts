...
const emitAuthoringEvent = (
    request: Parameters<Controller>[0],
    emit: EmitServerEvent,
    event: Extract<ServerEventInput, { source: 'mcp' }>,
) => {
    if (request.integration && request.integration.connectionId !== 'mcp') {
        const shared = {
            noteId: event.noteId,
            source: 'integration' as const,
            connectionId: request.integration.connectionId,
        };
        if (event.type === 'mcp.note.deleted') emit({ ...shared, type: 'integration.note.deleted' });
        else
            emit({
                ...shared,
                type: event.type === 'mcp.note.created' ? 'integration.note.created' : 'integration.note.updated',
...
export const createCreateNoteHandler = (
    createNote = createNoteFromMarkdown,
    emitEvent: EmitServerEvent = emitServerEvent,
): Controller => {
    return async (req, reply) => {
        const { title, markdown, layout, properties } = req.body ?? {};
        const resolvedProperties = resolveMetadataPropertyPatch(properties);
        if (resolvedProperties?.deleteKeys !== undefined)
            throw createAppError(400, 'INVALID_NOTE_PROPERTIES', 'Create accepts properties.set only.');
        ...
        try {
            const note = await createNote({
                ...
                ...(resolvedProperties ? { properties: resolvedProperties } : {}),
                ...(resolvedLayout ? { layout: resolvedLayout } : {}),
            });

            emitAuthoringEvent(req, emitEvent, {
            ...
            if (error instanceof InvalidNotePropertyInputError)
                throw createAppError(400, 'INVALID_NOTE_PROPERTIES', error.message);
...
export const createNoteWriteBaselineHandler = (
...
export const createPatchNoteMarkdownHandler = (
...
export const createAppendNoteMarkdownHandler = (
...
export const createReplaceNoteMarkdownHandler = (
...
export const createUpdateNoteMetadataHandler = (
...
export const createDeleteNoteHandler = (
