...
import { subscribeNoteChanges } from '../src/features/note/services/change-events.js';
...
test('property schema edits invalidate notes without changing their edit timestamps', async (t) => {
    const events: Array<{ type: string; noteId: number }> = [];
    const unsubscribe = subscribeNoteChanges((event) => events.push({ type: event.type, noteId: event.noteId }));
    t.after(unsubscribe);
    ...
    const definition = await models.propertyDefinition.create({
        data: { key: 'status', name: 'Status', valueType: 'text' },
    });
    ...
    await updateNotePropertyDefinition({ key: 'status', input: { name: 'Workflow status' } });
    ...
    assert.deepEqual(events, [{ type: 'note.updated', noteId: note.id }]);
});
