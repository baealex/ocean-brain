...
import { Empty, FallbackRender, NoteFilters, Pagination, Skeleton } from '~/components/shared';
...
export default function Home() {
    ...
    return (
        <>
            ...
            <NoteFilters
                viewMode={viewMode}
                onViewModeChange={(mode) => updateSearchParams({ view: mode, page: '1' })}
                itemsPerPage={limit}
                onItemsPerPageChange={(count) => updateSearchParams({ limit: count.toString(), page: '1' })}
                sortBy={sortBy}
                onSortByChange={(sort) => updateSearchParams({ sortBy: sort, page: '1' })}
                sortOrder={sortOrder}
                onSortOrderChange={(order) => updateSearchParams({ sortOrder: order, page: '1' })}
            />
            <Suspense
                fallback={(
